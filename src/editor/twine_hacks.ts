...
export function getEditorInstances(): Editor[] {
  return Array.from(document.getElementsByClassName("CodeMirror"))
    .map((x) => (x as any).CodeMirror as Editor)
    .filter((x) => x);
}

export function getNamespace(): string {
  return document.getElementsByClassName("passage-map").length == 0
    ? PACKAGE.name
...
export function init() {
  ...
  window.CodeMirror ??= Object.getPrototypeOf(
    (document.getElementsByClassName("CodeMirror")[0] as any).CodeMirror
  ).constructor;
}
